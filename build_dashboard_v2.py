  ...
  <div class="section" id="sec-dq">
    <div class="card">
      ...
      <div style="display:flex;flex-direction:column;gap:14px;font-size:.81rem;line-height:1.7;margin-top:8px">
        ...
        <div><strong>5. First-Pass Rate Methodology</strong><br/>An invoice is "first-pass" if its Document ID does <em>not</em> appear in the inc exceptions dataset for ANY exception (including Exc 0 and Exc 91). Denominator = 2025 Invoices US universe (Posted + Regular) with the two flagged vendors (Southern Ionics, Terra First) removed so the KPI matches the filtered charts. Overall rate: 14.9% (2,261 of 15,157).</div>
        <div><strong>6. Invoice Universe (Denominator)</strong><br/>Source: 2025 Invoices US file filtered to Status = "Posted" and PO Type = "Regular" → 16,580 unique invoices, then minus Southern Ionics + Terra First (1,423 invoices) = <strong>15,157 invoices</strong> used as the first-pass denominator. Transport vendors (supplier IDs starting "52") are excluded from exception <em>analysis</em> but their inc-exception rows still disqualify their universe invoices from first-pass.</div>
